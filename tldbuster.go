...
func main() {
	...
	// Print banner in cyan color if not in silent mode
	if !*silent {
		printBanner()
	}
	...
	// TLD list (replace with the full list as per your TLDs)
	tldList := []string{
...
"zara", "zero", "zip", "zm", "zone", "zuerich", "zw",
	...
	// Convert TLDs to lowercase
	for i, tld := range tldList {
		tldList[i] = strings.ToLower(tld)
	}

	var results []Result

	for _, domain := range domains {
		baseName, originalTLD := extractBaseName(domain, tldList)
		if baseName == "" {
			if *debug {
				fmt.Printf("Could not extract base name from domain: %s\n", domain)
			}
			continue
		}
		if *debug {
			fmt.Printf("Base name: %s, Original TLD: %s\n", baseName, originalTLD)
		}

		// Check if the original domain exists
		if exists, _ := checkDomain(domain); !exists {
			if *debug {
		...
		for _, tld := range tldList {
			...
			if exists, ips := checkDomain(newDomain); exists {
				registrant, server := performWhois(newDomain)
				result := Result{
					Domain:     newDomain,
					IPs:        ips,
...
// printBanner prints the banner in cyan color
func printBanner() {
	cyan := "\033[36m"
	reset := "\033[0m"
	banner := `
...
// extractBaseName extracts the base domain name (without TLD)
func extractBaseName(domain string, tldList []string) (string, string) {
	domain = strings.ToLower(domain)
	labels := strings.Split(domain, ".")
	for i := 1; i <= len(labels); i++ {
		possibleTLD := strings.Join(labels[len(labels)-i:], ".")
		if contains(tldList, possibleTLD) {
...
// checkDomain checks if a domain exists by resolving its IP addresses
...
// performWhois performs a WHOIS lookup for a domain
func performWhois(domain string) (string, string) {
	tld := getTLD(domain)
	...
	registrant := extractField(result, "Registrant Name:")
	if registrant == "" {
		registrant = extractField(result, "Admin Name:")
	}
	server := extractField(result, "Registrar:")
...
// outputResults writes the results to a file in JSON or TXT format
